  ...
  setup() {
    ...
    const onSubmit = ()=>{
      ...
      if(state.selectData.nikeName || state.selectData.role){
        ...
        arr = state.listData.filter( (v:any) =>v.nikeName.indexOf(state.selectData.nikeName) !== -1)
        }
        if(state.selectData.role){
          arr = (state.selectData.nikeName? arr :state.listData).filter((v:any)=>v.role.find((i:any)=> i.role === state.selectData.role))
        }
      }
      else{
        // 空直接返回全部数据
